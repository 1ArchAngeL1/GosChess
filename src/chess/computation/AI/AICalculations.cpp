...
static std::vector<GosChess::Move> MergeColorMoves(const unsigned char *board, GosChess::Color color) {
    GosChess::Figure fig;
    ...
    for (auto &figure_moves: GosChess::game_available_moves) {
        for (auto move: figure_moves.second) {
            ...
            if (fig.color == color) moves.push_back(move);
        }
    }
    return moves;
}
...
static int Search(GosChess::Board &board, int depth, bool maximize, int alpha, int beta, bool initial, GosChess::Move &best_move) {
    ...
    OrderMoves(board.GetRawBoard(), moves);
    int evaluated_best = maximize ? INT_MIN : INT_MAX;
    GosChess::Move curr_best_move;
    for (auto &move: moves) {
        GosChess::CalculateAvailableMoves(board.GetRawBoard(), color_to_calculate);
        if (MakeMove(move, board, color_to_calculate)) {
...
GosChess::Move GosChess::GetBestMove(GosChess::Board board) {
    ...
    sf::Clock clock;
    Search(board, 4, true, INT_MIN, INT_MAX, true, best_move);
